...
type ListResp = { ok: boolean; data?: Row[]; page?: number; pageSize?: number; total?: number; error?: string; };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="rounded-md bg-black text-white px-3 py-2 text-sm" onClick={() => setShowAdd(true)}>
          + Tambah Stok
        </button>
      </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari (produk/SN/IMEI/storage/warna)"
        ...
        <input value={to} onChange={(e) => setTo(e.target.value)} placeholder="dd/mm/yyyy"
               className="border rounded px-3 py-2 text-sm w-[120px]" />
        <button onClick={() => setPage(1)} className="border rounded px-3 py-2 text-sm">Terapkan</button>
      </div>
      ...
      <div className="border rounded-md overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td className="p-3 text-center" colSpan={10}>Memuat...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td className="p-3 text-center" colSpan={10}>Tidak ada data.</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="[&>td]:p-2 border-t">
                  ...
                  <td className="text-center">
                    <div className="inline-flex gap-2">
                      <button className="border px-2 py-1 rounded" onClick={() => setEditRow(r)}>Edit</button>
                      <button className="border px-2 py-1 rounded text-red-600" onClick={() => setDeleting(r.id)}>Hapus</button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <div className="flex items-center justify-between mt-3 text-sm">
        <div>Total {total} data • Hal {page} / {Math.max(1, Math.ceil(total / pageSize))}</div>
        <div className="flex gap-2">
          <button className="border px-3 py-1 rounded disabled:opacity-50" disabled={page <= 1}
                  onClick={() => setPage(p => Math.max(1, p - 1))}>Prev</button>
          <button className="border px-3 py-1 rounded disabled:opacity-50" disabled={page >= Math.max(1, Math.ceil(total / pageSize))}
                  onClick={() => setPage(p => p + 1)}>Next</button>
        </div>
      </div>

      {/* Modals */}
      {showAdd && <AddModal onClose={() => setShowAdd(false)} onSaved={() => { setShowAdd(false); void load(); }} />}
      {editRow && <EditModal row={editRow} onClose={() => setEditRow(null)} onSaved={() => { setEditRow(null); void load(); }} />}
      {deleting != null && <DeleteModal id={deleting} onClose={() => setDeleting(null)} onDeleted={() => { setDeleting(null); void load(); }} />}
    </div>
  );
}
...
function AddModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const body = {
        nama_produk: nama,
        sn, imei, storage, warna, garansi, asal_produk: asal,
      ...
      <div className="bg-white rounded-xl p-4 w-[720px] max-w-[95vw]">
        ...
        <form onSubmit={onSubmit} className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 flex justify-end gap-2">
            <button type="button" onClick={onClose} className="border px-4 py-2 rounded">Batal</button>
...
function EditModal({ row, onClose, onSaved }: { row: Row; onClose: () => void; onSaved: () => void }) {
  ...
  const [modal, setModal]     = useState(row.harga_modal != null ? String(row.harga_modal) : "");
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const body = {
        nama_produk: nama, sn, imei, storage, warna, garansi, asal_produk: asal,
        harga_modal: Number(modal || 0),
        tanggal_masuk: dmyToISO(tgl),
      };
...
function DeleteModal({ id, onClose, onDeleted }: { id: number; onClose: () => void; onDeleted: () => void }) {
